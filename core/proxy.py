    ...
    def __init__(
        ...
        if proxy_file and os.path.isfile(proxy_file):
            ...
            with open(
                ...
                for line in f:
                    ...
                    if "://" not in line:
                        line = "http://" + line
                    if line not in self._all:
                        self._all.append(line)
                        loaded += 1
            if loaded:
                info(f"Loaded {loaded} proxies from {proxy_file}")
        ...
        if proxy_url:
            ...
            if u:
                if "://" not in u:
                    u = "http://" + u
                if u not in self._all:
                    self._all.insert(0, u)

        self._all = list(dict.fromkeys(self._all))

        if validate and self._all:
            self.validate_all(timeout=8)
    ...
    def fetch_fresh(self, count=200):
        ...
        for p in fresh:
            if (
                ...
                existing.add(p["url"])
                added += 1
                if p["url"] not in self._all:
                    self._all.append(p["url"])
        self._alive.sort(key=lambda x: x["latency"])
        self._last_fetch = time.time()
        self._consecutive_dead = 0
        ok(
