...
class OSINT:
    ...
    def _via_web_profile_info(self):
        ...
        if not self._is_real():
            ...
            raw_str = r.text
            self._parse_any_json_blob(raw_str, self.username)
        return True

    def _via_i_api(self):
        url = f"https://i.instagram.com/api/v1/users/web_profile_info/?username={self.username}"
        h = {"User-Agent": Session.ig_app_ua(), "X-IG-App-ID": self.APP_ID, "Accept": "*/*", "Accept-Language": random.choice(Session.LANGS)}
        h.update(Session.new_device_ids())
        r = self.session.get(url, headers=h, timeout=25)
        if r.status_code != 200:
    ...
    def _via_html(self):
        ...
        # Method A: window._sharedData (old format)
        ...
        # Method B: additionalDataLoaded (injected JSON)
        ...
        # Method C: any JSON blob containing username
        if self._parse_any_json_blob(html, self.username):
            if self._is_real():
                return True

        # Method D: RSC / Server Components format (newer Instagram)
        if self._parse_rsc(html):
            if self._is_real():
                return True

        # Method E: OG meta tags
        if self._parse_og(html):
            return True

        if "login" in html.lower() and "window._sharedData" not in html:
    ...
    def _parse_any_json_blob(self, html, username):
        """Find ANY JSON blob in HTML that contains the username and extract user data."""
        ...
        # Extract a window around the username
        start = max(0, pos - 15000)
        end = min(len(html), pos + 15000)
        chunk = html[start:end]

        # Try to find structured user data
        found_any = False

        # Look for user dict pattern
        for pattern_key in ["full_name", "biography", "edge_followed_by", "follower_count", "is_private", "is_verified", "profile_pic_url", "edge_owner_to_timeline_media", "media_count"]:
    ...
    def _parse_rsc(self, html):
        """Parse React Server Components format."""
        # Look for JSON-like structures embedded in RSC payloads
        found = False
        for pattern in [
            r'"username":"' + re.escape(self.username) + r'"[^}]*"full_name":"([^"]*)"',
        ...
        prv = re.search(r'"is_private"\s*:\s*(true|false)', html)
        d["is_private"] = bool(prv and prv.group(1) == "true")
        ver = re.search(r'"is_verified"\s*:\s*(true|false)', html)
        ...
        if d["full_name"] or d["biography"] or d["id"] or d["followers"] > 0 or d["posts"] > 0:
            self.data = d
            return True
        return False
    ...
    def _parse_og(self, html):
        ...
        self.data = {
            ...
            "is_private": priv, "is_verified": False, "is_business": False,
    ...
    def _save_empty(self):
        self.data = {
            ...
            "osint_ok": False, "fail_reason": self.fail_reason or "no public data",
        }
        self._save()
...
""")
    ...
    def get_hints(self):
        ...
        return {
            ...
            "bio_tokens": list(dict.fromkeys(tokens)), "years": list(dict.fromkeys(years)),
            "phones": phones, "followers": int(self.data.get("followers") or 0),
            "following": int(self.data.get("following") or 0), "posts": int(self.data.get("posts") or 0),
            "user_parts": parts, "stat_numbers": list(dict.fromkeys(stats)),
