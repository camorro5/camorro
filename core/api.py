...
class InstagramAPI:
    ...
    def set_proxy(self, proxy_url):
        ...
        else: self.proxy = None; self.session.proxies.clear()
    ...
    def profile_exists(self, username):
        ...
        try:
            ...
            if r.status_code == 200:
                t = r.text or ""
                if "Sorry, this page isn't available" in t: return False
                return True
    ...
    def try_login(self, username, password):
        ...
        try:
            ...
            ts = int(time.time())
            data = {"username": username, "enc_password": f"#PWD_INSTAGRAM_BROWSER:0:{ts}:{password}",
            ...
            if j.get("authenticated") is True: return {"success": True, "status": "ok", "raw": j}
            if j.get("user") is False: return {"success": False, "status": "invalid_user", "raw": j}
            ...
            if j.get("authenticated") is False: return {"success": False, "status": "bad_password", "raw": j}
            if j.get("status") == "fail":
                if "password" in msg or r.status_code == 400: return {"success": False, "status": "bad_password", "raw": j}
                return {"success": False, "status": "error", "raw": j}
            return {"success": False, "status": "bad_password", "raw": j}
        except requests.exceptions.Timeout: return {"success": False, "status": "timeout"}
        except requests.exceptions.ProxyError: return {"success": False, "status": "connection_error"}
        except requests.exceptions.ConnectionError: return {"success": False, "status": "connection_error"}
        except Exception as e: return {"success": False, "status": "error", "error": str(e)}
