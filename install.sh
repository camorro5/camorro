...
install_python_deps() {
    ...
    # FIXED FOR TERMUX: Do NOT upgrade pip inside Termux
    if check_termux; then
        echo -e "${YELLOW}[!] Termux detected: Skipping pip upgrade (would break system package)[/yellow]${NC}"
    else
        # Only upgrade pip on normal Linux
    ...
    pip install xmlsec==1.3.14 --quiet || {
        echo -e "${YELLOW}[!] xmlsec failed (common in Termux), continuing...${NC}"
    }
    ...
    # Try transformers (might fail on Termux)
